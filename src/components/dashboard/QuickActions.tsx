...
const ALL_ACTIONS: QuickAction[] = [
  { id: 'view-members', label: 'Members', icon: <Users size={20} /> },
  { id: 'add-coupon', label: 'Coupon', icon: <Ticket size={20} /> },
  { id: 'create-campaign', label: 'Campaign', icon: <Megaphone size={20} /> },
  { id: 'view-reports', label: 'Reports', icon: <BarChart3 size={20} /> },
  { id: 'manage-tiers', label: 'Tiers', icon: <Star size={20} /> },
  { id: 'rewards-setup', label: 'Rewards', icon: <Gift size={20} /> },
      ...
      <div className="flex items-center gap-2 p-2 bg-white/80 backdrop-blur-xl border border-white/20 shadow-2xl rounded-full">
        ...
        {activeActions.map((action) => (
          <div key={action.id} className="group relative">
             <button className="p-3 rounded-full hover:bg-white text-slate-600 hover:text-primary-600 transition-all hover:scale-110 hover:shadow-lg relative">
               {action.icon}
               {isEditMode && (
                 <div 
        ...
        <button 
          ...
          className={`p-3 rounded-full transition-all ${
            isEditMode ? 'bg-slate-900 text-white rotate-45' : 'hover:bg-white text-slate-400 hover:text-slate-600'
          }`}
        >
          <Plus size={20} />
        </button>
