...
const CampaignEditor: React.FC<CampaignEditorProps> = ({ onNavigate }) => {
    ...
    const [targetStores, setTargetStores] = useState<string[]>(draftCampaign?.targetStores || []);
    const [targetTiers, setTargetTiers] = useState<string[]>(draftCampaign?.targetTiers || []);
    const [stackable, setStackable] = useState<boolean>(draftCampaign?.stackable ?? false);
    ...
    const persistCampaign = (status: 'active' | 'draft' | 'paused' | 'scheduled') => {
        ...
        const finalCampaign: Campaign = {
            ...
            totalParticipants: existingCampaign?.totalParticipants || 0,
    ...
    }, [targetTiers, productConditions, isBatchDrop]);
        ...
        return (
            <div key={reward.id} className="relative group/item">
                ...
                <div className={`relative p-3 bg-white border rounded-xl flex items-center gap-3 shadow-sm z-10 ${warning ? 'border-amber-200 rounded-tl-none' : 'border-slate-200'}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border shrink-0 ${reward.type === 'coupon' ? 'bg-purple-100 text-purple-600 border-purple-200' :
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg">
                                                ...
                                                <select
                                                    ...
                                                    value={targetTiers.length === 0 ? 'All Tiers' : targetTiers[0]}
                                                    onChange={(e) => setTargetTiers(e.target.value === 'All Tiers' ? [] : [e.target.value])}
                                                >
                                ...
                                <div>
                                    <div className="flex flex-col gap-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-sm font-bold text-slate-900">{isBatchDrop ? 'Include members from' : 'Valid at Stores'}</span>
                                        </div>
                                        <div className="flex flex-wrap gap-2 pt-1">
                                            <button
                                                onClick={() => setTargetStores([])}
                                                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${targetStores.length === 0 ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
                                            >
                                                All Stores
                                            </button>
                                            {STORES.map(s => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        if (targetStores.includes(s)) {
                                                            setTargetStores(targetStores.filter(item => item !== s));
                                                        } else {
                                                            setTargetStores([...targetStores, s]);
                                                        }
                                                    }}
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${targetStores.includes(s) ? 'bg-primary-500 text-white border-primary-500 shadow-sm shadow-primary-200' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
                                                >
                                                    {s}
                                                </button>
                                            ))}
                                    ...
                                    <div>
                                        <div className="flex flex-col gap-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-sm font-bold text-slate-900">For Member Levels</span>
                                            </div>
                                            <div className="flex flex-wrap gap-2 pt-1">
                                                <button
                                                    onClick={() => setTargetTiers([])}
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${targetTiers.length === 0 ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
                                                >
                                                    All Tiers
                                                </button>
                                                {TIERS.map(t => (
                                                    <button
                                                        key={t}
                                                        onClick={() => {
                                                            if (targetTiers.includes(t)) {
                                                                setTargetTiers(targetTiers.filter(item => item !== t));
                                                            } else {
                                                                setTargetTiers([...targetTiers, t]);
                                                            }
                                                        }}
                                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${targetTiers.includes(t) ? 'bg-primary-500 text-white border-primary-500 shadow-sm shadow-primary-200' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
                                                    >
                                                        {t}
                                                    </button>
                                                ))}
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Stacking Rules</h3>
                                        </div>
                                        <button
                                            onClick={() => setStackable(!stackable)}
                                            className={`w-12 h-6 rounded-full p-1 transition-all ${stackable ? 'bg-emerald-500' : 'bg-slate-200'}`}
                                        >
                                            <div className={`w-4 h-4 bg-white rounded-full transition-transform ${stackable ? 'translate-x-6' : 'translate-x-0'}`} />
                                        </button>
                                    </div>
                                    <p className="text-xs text-slate-500 mb-4 font-medium">Allow this campaign to be used in conjunction with other active campaigns.</p>

                                    {stackable && campaigns.filter(c => c.status === 'running' && c.id !== draftCampaign.id).length > 0 && (
                                        <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl animate-in zoom-in-95 duration-200">
                                            <div className="flex items-start gap-3">
                                                <AlertTriangle size={18} className="text-amber-600 shrink-0 mt-0.5" />
                                                <div>
                                                    <div className="text-sm font-bold text-amber-900 mb-1">Overlapping Campaigns Detected</div>
                                                    <div className="text-xs text-amber-700 leading-relaxed font-medium">
                                                        Enabling stacking may conflict with these active campaigns:
                                                        <ul className="mt-2 space-y-1 list-disc list-inside opacity-80">
                                                            {campaigns.filter(c => c.status === 'running' && c.id !== draftCampaign.id).map(c => (
                            ...
                            <div>
                                ...
                                <ul className="space-y-2">
                                    <li className="flex gap-2">
                                        ...
                                        <span>Store: <strong className="text-white">{targetStores.length === 0 ? 'All Stores' : targetStores.join(', ')}</strong></span>
                                    </li>
                                    <li className="flex gap-2">
                                        <Users size={14} className="mt-0.5 text-purple-400 shrink-0" />
                                        <span>Audience: <strong className="text-white">{targetTiers.length === 0 ? 'All Tiers' : targetTiers.join(', ')}</strong></span>
                                    </li>
                                    {stackable && (
                                        <li className="flex gap-2">
                                            <Layers size={14} className="mt-0.5 text-emerald-400 shrink-0" />
                                            <span>Stacking: <strong className="text-white">Enabled</strong></span>
                                        </li>
                                    )}
                                    {showProductScope && (
                                        <li className="flex gap-2">
