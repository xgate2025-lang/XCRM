...
import { 
  ...
  Crown, Lock, ChevronDown, Check, Coins, Calendar, ArrowUp, AlertCircle, Image as ImageIcon, UploadCloud, Plus, Tag, Truck, Star, RotateCw, UserCheck, Unlock,
  Hash, Bookmark, Users, TrendingUp, TrendingDown, Clock, AlertTriangle, ShieldCheck, RefreshCw, Scale
} from 'lucide-react';
...
const TierWizard: React.FC<TierWizardProps> = ({ programLogic, initialData, onClose, onSave, programStatus, tiers }) => {
  ...
  // Custom Benefit Templates
  const [customTemplates, setCustomTemplates] = useState<string[]>([
    'Priority Support', 'Valet Parking', 'Concierge Service'
  ]);
  
  // New State for "Add Privilege" Dropdown
  const [isPrivilegeMenuOpen, setIsPrivilegeMenuOpen] = useState(false);
  ...
  const [tierData, setTierData] = useState<TierDefinition>(() => {
    ...
    return {
        ...
        multiplier: 1.5,
        benefits: [
            { id: 'ben_init_1', category: 'ongoing', type: 'free_shipping', label: 'Standard Free Shipping', value: 'Standard' }
        ]
    };
  });
  ...
  // Specialized Add Handlers for Dropdown
  const addPrivilege = (type: 'discount' | 'free_shipping' | 'custom', templateValue?: string) => {
    const id = Math.random().toString(36).substr(2, 9);
    let newBenefit: TierBenefit;

    switch(type) {
        case 'discount':
            newBenefit = { id, category: 'ongoing', type: 'discount', value: '5', label: '5% Off' };
            break;
        case 'free_shipping':
            newBenefit = { id, category: 'ongoing', type: 'free_shipping', value: 'Standard', label: 'Standard Shipping' };
            break;
        case 'custom':
            newBenefit = { 
                id, 
                ...
                type: 'custom', 
                value: templateValue || '', 
                label: templateValue || 'Priority Support' 
            };
            break;
        default:
            return;
  ...
  const renderPreviewCard = () => {
    ...
    return (
      <div className="w-full max-w-xs mx-auto animate-in fade-in slide-in-from-right-8 duration-500 sticky top-10">
        ...
        {/* Device Simulation */}
        <div className="bg-white rounded-[2.5rem] shadow-2xl border-8 border-slate-900 overflow-hidden relative min-h-[600px]">
            {/* Status Bar */}
            ...
            {/* Content Scroll */}
            <div className="p-4 space-y-6 overflow-y-auto h-[500px] no-scrollbar">
                
                {/* The Card */}
                <div 
                    className={`relative aspect-[1.586] rounded-2xl shadow-xl overflow-hidden transition-all duration-500 bg-cover bg-center ${cardBgClass} ${textColorClass}`}
                ...
                <div className="space-y-3">
                    ...
                    {/* Dynamic Benefits */}
                    {tierData.benefits.filter(b => b.category === 'ongoing').map(ben => (
                ...
                <div className="space-y-3">
                    ...
                    {tierData.benefits.filter(b => ['birthday', 'anniversary'].includes(b.category)).map(ben => (
                        <div key={ben.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                            <div className="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center text-pink-500">
                                {ben.category === 'birthday' ? <Gift size={14} /> : <Calendar size={14} />}
                            </div>
                            <div>
                                <div className="text-sm font-bold text-slate-700">{ben.label}</div>
                                <div className="text-[10px] text-slate-400 capitalize">{ben.category} Treat</div>
                            </div>
                        </div>
                    ))}
                     {tierData.benefits.filter(b => ['upgrade'].includes(b.category)).map(ben => (
                        <div key={ben.id} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                            ...
                            <div>
                                <div className="text-sm font-bold text-slate-700">{ben.label}</div>
                                <div className="text-[10px] text-slate-400">Welcome Bonus</div>
                            </div>
                        </div>
                    ))}
                </div>
                                        ...
                                        <div className="pt-6 border-t border-slate-100">
                                             ...
                                             <div className="grid grid-cols-1 gap-4">
                                                ...
                                                <div 
                                                    ...
                                                    className={`relative p-5 rounded-2xl border-2 transition-all cursor-pointer flex items-start gap-5 ${
                                                        tierData.downgradeLogic === 'soft_landing' 
                                                ...
                                                <div 
                                                    ...
                                                    className={`relative p-5 rounded-2xl border-2 transition-all cursor-pointer flex items-start gap-5 ${
                                                        tierData.downgradeLogic === 'hard_reset' 
                                                        ? 'border-primary-500 bg-primary-50' 
                                                        : 'border-slate-200 bg-white hover:border-slate-300'
                                                    } ${isCriticalLocked ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                >
                                                ...
                                                <div 
                                                    ...
                                                    className={`relative p-5 rounded-2xl border-2 transition-all cursor-pointer flex items-start gap-5 ${
                                                        tierData.downgradeLogic === 'dynamic_matching' 
                                                        ? 'border-primary-500 bg-primary-50' 
                                                        : 'border-slate-200 bg-white hover:border-slate-300'
                                                    } ${isCriticalLocked ? 'opacity-60 cursor-not-allowed' : ''}`}
                                                >
                            ...
                            <div className="px-6 pb-8 pl-[4.5rem] space-y-8 animate-in slide-in-from-top-2 duration-300">
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {/* 2. Dynamic Sentence Strips */}
                                        {tierData.benefits.filter(b => b.category === 'ongoing').map(ben => (
                                            <div key={ben.id} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl group hover:border-slate-300 transition-all">
                                                
                                                {/* Icon Logic */}
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                                    ben.type === 'discount' ? 'bg-green-100 text-green-600' : 
                                                    ben.type === 'free_shipping' ? 'bg-blue-100 text-blue-600' :
                                                    'bg-purple-100 text-purple-600'
                                                }`}>
                                                    {ben.type === 'discount' && <Tag size={14} />}
                                                    {ben.type === 'free_shipping' && <Truck size={14} />}
                                                    {ben.type === 'custom' && <Star size={14} />}
                                                </div>
                                                ...
                                                <div className="flex-1 font-medium text-slate-700">
                                                    {ben.type === 'discount' && (
                                                        <>
                                                            ...
                                                            <SentenceInput 
                                                                value={ben.value} 
                                                                onChange={(v) => updateBenefit(ben.id, { value: v, label: `${v}% Off` })} 
                                                                width="w-12" 
                                                                className="mx-1" 
                                                            /> 
                                                            % Off every order automatically.
                                                        </>
                                                    )}

                                                    {ben.type === 'free_shipping' && (
                                                        <>
                                                            Get 
                                                            <SentenceInput 
                                                                value={ben.value} 
                                                                onChange={(v) => updateBenefit(ben.id, { value: v, label: `${v} Shipping` })} 
                                                                width="w-24" 
                                                                className="mx-1" 
                                                            /> 
                                                    ...
                                                    {ben.type === 'custom' && (
                                                        <>
                                                            ...
                                                            <SentenceInput 
                                                                value={ben.label} 
                                                                onChange={(v) => updateBenefit(ben.id, { label: v, value: v })} 
                                                                width="w-40" 
                                                                className="mx-1" 
                                                ...
                                                <div className="flex items-center gap-1">
                                                    {/* Save Template Button for Custom */}
                                                    {ben.type === 'custom' && ben.label && !customTemplates.includes(ben.label) && (
                                                        <button 
                                                            onClick={() => setCustomTemplates(prev => [...prev, ben.label])}
                                                    ...
                                                    <button onClick={() => removeBenefit(ben.id)} className="text-slate-300 hover:text-red-500 p-1.5 rounded-lg hover:bg-red-50 transition-colors">
                                                        <X size={16}/>
                                                    </button>
                                                </div>
                                            </div>
                                        ...
                                        {/* Add Button Dropdown */}
                                        <div className="relative pt-2" ref={privilegeMenuRef}>
                                            <button 
                                                onClick={() => setIsPrivilegeMenuOpen(!isPrivilegeMenuOpen)}
                                                className="text-sm font-bold text-primary-600 hover:text-primary-700 flex items-center gap-1 pl-2 bg-white rounded-lg py-1 px-2 border border-transparent hover:border-slate-100 hover:shadow-sm transition-all"
                                            >
                                                <PlusIcon /> Add Privilege <ChevronDown size={14} />
                                            </button>

                                            {isPrivilegeMenuOpen && (
                                                <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-200">
                                                    <div className="px-4 py-2 bg-slate-50 border-b border-slate-100">
                                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Type</span>
                                                    </div>
                                                    <button 
                                                        onClick={() => addPrivilege('discount')}
                                                        className="w-full text-left px-4 py-3 hover:bg-slate-50 flex items-center gap-3 transition-colors"
                                                    >
                                                        <div className="w-8 h-8 rounded-full bg-green-50 text-green-600 flex items-center justify-center"><Tag size={16} /></div>
                                                        <span className="text-sm font-bold text-slate-700">Money Off</span>
                                                    </button>
                                                    <button 
                                                        onClick={() => addPrivilege('free_shipping')}
                                                        className="w-full text-left px-4 py-3 hover:bg-slate-50 flex items-center gap-3 transition-colors"
                                                    >
                                                    ...
                                                    <button 
                                                        onClick={() => addPrivilege('custom')}
                                                        className="w-full text-left px-4 py-3 hover:bg-slate-50 flex items-center gap-3 transition-colors"
                                                    >
                                                        <div className="w-8 h-8 rounded-full bg-purple-50 text-purple-600 flex items-center justify-center"><Star size={16} /></div>
                                                        <span className="text-sm font-bold text-slate-700">Custom Perk</span>
                                                    </button>
                                                        ...
                                                        <>
                                                            ...
                                                            <div className="max-h-40 overflow-y-auto">
                                                                {customTemplates.map((tmpl) => (
                                                                    <button
                                                                        key={tmpl}
                                                                        onClick={() => addPrivilege('custom', tmpl)}
                                                                        className="w-full text-left px-4 py-2 hover:bg-slate-50 flex items-center gap-2 transition-colors group"
                                                                    >
                                                                        <Star size={14} className="text-purple-400 group-hover:text-purple-600" />
                                                                        <span className="text-sm font-medium text-slate-600 group-hover:text-slate-900">{tmpl}</span>
                                                                    </button>
                                                                ))}
                                                            </div>
                                                        </>
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1 pt-6 border-t border-slate-100">
                                        ...
                                        <h4 className="text-sm font-bold text-slate-900 uppercase tracking-wider">
                                            {isBaseTier ? 'Onboarding Missions' : 'Lifecycle Events'}
                                        </h4>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5 space-y-4">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-3 mb-2">
                                                        ...
                                                        <span className="font-bold text-slate-900 text-sm">Welcome Gift</span>
                                                    </div>
                                                ...
                                                <div>
                                                    ...
                                                    {renderRewardList('profile')}
                                                    <button 
                                                        onClick={() => setShowRewardModal('profile')}
                                                        className="text-xs font-bold text-primary-600 hover:text-primary-700 ml-9 flex items-center gap-1"
                                                    >
                                                        <PlusIcon /> Add Reward
                                                    </button>
                                        ...
                                        {/* SCENARIO C: Calendar Triggers (All Tiers) */}
                                        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-5">
                                            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Calendar Triggers</div>
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex gap-4">
                                                    ...
                                                    <div>
                                                        <div className="font-bold text-slate-900">Birthday Treat</div>
                                                        <div className="text-xs text-slate-400 mt-1">Sent on member's Date of Birth</div>
                                                        <div className="mt-2 min-w-[200px]">
                                                            {renderRewardList('birthday')}
                                                            {!tierData.benefits.some(b => b.category === 'birthday') && (
                                                                <button 
                                                                    onClick={() => setShowRewardModal('birthday')}
                                                                    className="text-xs font-bold text-slate-400 hover:text-primary-600 border border-dashed border-slate-300 px-3 py-1.5 rounded-lg hover:border-primary-300 transition-all"
                                                                >
                                                                    + Add Treat
                                                                </button>
                                                            )}
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div className="flex gap-4">
                                                    ...
                                                    <div>
                                                        <div className="font-bold text-slate-900">Anniversary Gift</div>
                                                        <div className="text-xs text-slate-400 mt-1">Sent annually on join date</div>
                                                        <div className="mt-2 min-w-[200px]">
                                                            {renderRewardList('anniversary')}
                                                            {!tierData.benefits.some(b => b.category === 'anniversary') && (
                                                                <button 
                                                                    onClick={() => setShowRewardModal('anniversary')}
                                                                    className="text-xs font-bold text-slate-400 hover:text-primary-600 border border-dashed border-slate-300 px-3 py-1.5 rounded-lg hover:border-primary-300 transition-all"
                                                                >
                                                                    + Add Gift
                                                                </button>
