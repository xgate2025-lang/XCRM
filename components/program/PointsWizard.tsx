...
import { 
  ...
  Trash2, Plus, AlertCircle, Info, ToggleLeft, ToggleRight, Calendar, Coins, TrendingUp, ShieldAlert, ShieldCheck, Activity,
...
const FinancialImpactSimulator = ({ policy }: { policy: PointsConfig['expirationPolicy'] }) => {
  ...
  const isSafe = policy === 'fixed_date';
  ...
  const styles = isHighRisk 
    ...
    : isSafe 
      ? { bg: 'bg-green-50', border: 'border-green-100', text: 'text-green-900', sub: 'text-green-600', stroke: '#22c55e', fill: 'rgba(34, 197, 94, 0.1)' }
  ...
  const getPath = () => {
    ...
    if (isSafe) return "M 0,60 L 60,20 L 60,60 L 120,20 L 120,60 L 180,20 L 180,60"; // Sawtooth
    return "M 0,60 C 40,60 60,20 200,20"; // Logistic curve / plateau
  };

  const getFillPath = () => {
    if (isHighRisk) return "M 0,60 L 200,5 L 200,60 Z";
    if (isSafe) return "M 0,60 L 60,20 L 60,60 L 120,20 L 120,60 L 180,20 L 180,60 Z";
    ...
    <div className={`mt-4 rounded-xl border ${styles.bg} ${styles.border} p-5 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden`}>
       ...
       <div className="flex-1 space-y-2 z-10">
           <div className="flex items-center gap-2">
               ...
               {isSafe && <ShieldCheck size={18} className="text-green-600" />}
               {isBalanced && <Activity size={18} className="text-blue-600" />}
               <h4 className={`font-bold text-sm uppercase tracking-wide ${styles.text}`}>
                   {isHighRisk ? 'Accumulating Debt' : isSafe ? 'Annual Reset' : 'Stabilized Liability'}
               </h4>
           </div>
           ...
           <p className={`text-sm ${styles.sub} font-medium leading-snug`}>
               ...
               {isSafe && "Liability is cleared completely once per year. Best for financial predictability, but may cause customer frustration on expiry day."}
...
const PointsWizard: React.FC<PointsWizardProps> = ({ 
            ...
            <div className="space-y-6 pt-4">
              ...
              <div>
                 <div className="flex items-center gap-3 mb-2">
                    <span className="flex items-center justify-center w-5 h-5 rounded-full bg-slate-200 text-slate-600 text-xs font-bold">1</span>
              ...
              {/* Condition B: Product Scope (Smart Filter) */}
              <div className="relative">
                {/* Connector Line */}
                <div className="absolute left-2.5 -top-4 bottom-0 w-0.5 bg-slate-200"></div>
                
                <div className="flex items-center gap-3 mb-3 relative z-10">
                   <span className="flex items-center justify-center w-5 h-5 rounded-full bg-slate-200 text-slate-600 text-xs font-bold">2</span>
                   <span className="text-slate-900 font-bold text-sm">What</span>
                </div>
                
                <div className="ml-8 space-y-2">
                   <div className="text-slate-500 font-medium text-sm mb-2">Cart includes items where...</div>
  ...
  const renderExpirationContent = () => {
    return (
      <div className="animate-in fade-in slide-in-from-top-2">
        ...
        {config.expirationPolicy === 'fixed_date' && (
          <div className="mt-6 space-y-6 pl-4 border-l-2 border-slate-100">
            <div className="text-lg text-slate-600 font-medium">
              Clear all point balances on 
              <select 
                className="mx-2 bg-slate-50 border-b-2 border-slate-200 font-bold text-slate-900 focus:outline-none focus:border-primary-500 py-1"
                value={config.fixedDateConfig?.month}
                onChange={(e) => updateConfig({ fixedDateConfig: { ...config.fixedDateConfig!, month: Number(e.target.value) } })}
              >
                {[...Array(12)].map((_, i) => <option key={i} value={i+1}>{new Date(0, i).toLocaleString('en-US', { month: 'short' })}</option>)}
              </select>
              <select 
                className="mx-2 bg-slate-50 border-b-2 border-slate-200 font-bold text-slate-900 focus:outline-none focus:border-primary-500 py-1"
                value={config.fixedDateConfig?.day}
                onChange={(e) => updateConfig({ fixedDateConfig: { ...config.fixedDateConfig!, day: Number(e.target.value) } })}
              >
                {[1, 15, 28, 30, 31].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              of every year.
            </div>
            
            <div className="bg-slate-50 p-4 rounded-xl">
              <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Grace Period</div>
              <div className="text-sm text-slate-600 font-medium">
                Warn members 
                <SentenceInput 
                  value={config.fixedDateConfig?.gracePeriodDays || 30}
                  onChange={(v) => updateConfig({ fixedDateConfig: { ...config.fixedDateConfig!, gracePeriodDays: Number(v) } })}
                  width="w-12"
                  className="mx-2"
                />
  ...
  const renderRedemptionExclusions = () => {
    ...
    const addGroup = () => {
        ...
        updateConfig({ exclusionRules: [...config.exclusionRules, newGroup] });
    };

    // Helper to remove a group
    const removeGroup = (groupId: string) => {
        updateConfig({ exclusionRules: config.exclusionRules.filter(g => g.id !== groupId) });
    };

    // Helper to update conditions within a group
    const updateGroupConditions = (groupId: string, newConditions: AttributeCondition[]) => {
        const updatedGroups = config.exclusionRules.map(g => 
            g.id === groupId ? { ...g, conditions: newConditions } : g
        );
        updateConfig({ exclusionRules: updatedGroups });
    };

    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
                <span className="text-sm font-bold text-slate-700">Exclude an item if it matches...</span>
            </div>

            {config.exclusionRules.length === 0 && (
                <div className="p-8 bg-slate-50 border border-dashed border-slate-300 rounded-xl text-center">
                    <p className="text-sm text-slate-500 font-medium mb-4">No specific items are excluded. All items are eligible.</p>
                    <button 
                        onClick={addGroup}
                        className="px-4 py-2 bg-white border border-slate-200 shadow-sm rounded-lg text-sm font-bold text-slate-700 hover:border-slate-300 hover:text-slate-900 transition-all"
                    >
                        + Add First Exclusion
                    </button>
                </div>
            ...
            <div className="space-y-4">
                {config.exclusionRules.map((group, index) => (
                    <React.Fragment key={group.id}>
                        {index > 0 && (
                            <div className="flex items-center gap-4">
                                <div className="h-px bg-slate-200 flex-1"></div>
                                <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded">OR</span>
                                <div className="h-px bg-slate-200 flex-1"></div>
                            </div>
                        )}
                        
                        <div className="p-5 bg-slate-50 border border-slate-200 rounded-xl relative group">
                            {/* Remove Group Button */}
                            <button 
                                ...
                                title="Remove this set of rules"
                            >
                                <X size={16} />
                            </button>

                            <div className="mb-3">
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Item Set #{index + 1}</span>
                            </div>

                            <SmartFilterBuilder 
                                conditions={group.conditions}
                                onChange={(newConditions) => updateGroupConditions(group.id, newConditions)}
                                logicMode="AND"
                            />
                        </div>
                    </React.Fragment>
            ...
            {config.exclusionRules.length > 0 && (
                <button 
                    onClick={addGroup}
                    className="w-full py-3 mt-4 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 font-bold hover:border-primary-300 hover:text-primary-600 hover:bg-primary-50 transition-all flex items-center justify-center gap-2"
                >
                    <Plus size={18} /> Add Another Item Set
                </button>
            )}
        </div>
    );
           ...
           <div className="max-w-3xl mx-auto">
               ...
               {activeTab === 0 && (
                   <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                       ...
                       <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between">
                           <div className="flex items-center gap-4">
                               <div className={`p-3 rounded-full ${config.enabled ? 'bg-primary-100 text-primary-600' : 'bg-slate-100 text-slate-400'}`}>
                                   <Coins size={24} />
                               </div>
                               <div>
                                   <h3 className="text-lg font-bold text-slate-900">Redeemable Points</h3>
                                   <p className="text-sm text-slate-500">The currency members redeem for coupons or products.</p>
                               </div>
                           </div>
                           <button onClick={() => updateConfig({ enabled: !config.enabled })} className="transition-colors">
                               {config.enabled 
               ...
               {activeTab === 1 && (
                   <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                       ...
                       <div 
                         onClick={() => updateConfig({ expirationPolicy: 'fixed_date' })}
                         className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${config.expirationPolicy === 'fixed_date' ? 'border-primary-500 bg-white shadow-lg' : 'border-slate-200 bg-white hover:bg-slate-50'}`}
                       >
                           <div className="flex items-start justify-between">
                               <div className="flex gap-4">
                                   <div className="p-3 bg-slate-100 rounded-xl text-slate-500">
                                   ...
                                   <div>
                                       <h3 className="text-lg font-bold text-slate-900">Fixed Calendar Date</h3>
                                       <p className="text-sm text-slate-500 mt-1">All points for all members expire on the same day every year.</p>
                                   </div>
                               </div>
                               <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${config.expirationPolicy === 'fixed_date' ? 'border-primary-500' : 'border-slate-300'}`}>
                                    {config.expirationPolicy === 'fixed_date' && <div className="w-3 h-3 rounded-full bg-primary-500" />}
                               </div>
                           </div>
                           {config.expirationPolicy === 'fixed_date' && renderExpirationContent()}
                       </div>

                       {/* Option C: Retail Standard */}
                       <div 
               ...
               {activeTab === 2 && (
                   <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                       ...
                       {/* 3. Product Restrictions (Exclusion Engine) */}
                       <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                           <div className="flex items-center justify-between mb-4">
                               <div>
                                   <h3 className="text-lg font-bold text-slate-900">Product Restrictions</h3>
                                   <p className="text-sm text-slate-500">Select items that cannot be purchased using points.</p>
                               </div>
                           </div>

                           <div className="space-y-6">
                               {/* Easy Button */}
                               <label className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100 cursor-pointer hover:bg-slate-100">
                                   <input 
                                    type="checkbox"
                                    checked={config.excludeGiftCards}
                                    onChange={(e) => updateConfig({ excludeGiftCards: e.target.checked })}
                                    className="w-5 h-5 text-primary-600 rounded"
                                   />
                                   <div>
                                       <span className="block text-sm font-bold text-slate-900">Exclude Gift Cards & Cash Equivalents</span>
                                       <span className="block text-xs text-slate-500">Highly Recommended. Prevents the 'Gift Card Loop'.</span>
                                   </div>
                               </label>

                               {/* Advanced Builder */}
                               <div>
                                   <h4 className="text-sm font-bold text-slate-900 mb-3">Advanced Exclusion Rules</h4>
                                   {renderRedemptionExclusions()}
                               </div>
                           </div>
                       </div>
